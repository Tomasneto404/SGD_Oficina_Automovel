...
class Alugueres(db.Model):
	id = db.Column(db.Integer, primary_key = True)
	cli_id = db.Column(db.Integer, nullable = False)
	vei_aluguer_id = db.Column(db.Integer, nullable = False)
	...
	diasAmais = db.Column(db.Integer, nullable = False, default = 0)
	kmFeitos = db.Column(db.Integer, nullable = False)
	portagens = db.Column(db.String(10), nullable = False)

	def __repr__(self):
		return 'Aluguer ' + str(self.id)
...
def oficina():
	...
	if current_user.cargo == 'Patrao':
		...
		if request.method == "POST":
			...
			#new_register = Oficina(nome = nome,  morada = morada, descricao = descricao, telefone = telefone, email = email, dono = dono)
			#db.session.add(new_register)
			db.session.commit()
...
def login():
	...
	return render_template("Software/pages/examples/login.html")
...
def register():
	...
	if exists:
		...
		return render_template("Software/pages/examples/register.html")
